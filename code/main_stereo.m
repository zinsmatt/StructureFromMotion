% Dans ce script nous utilisons deux images stéréo issues des deux caméras
% couleurs.
...
% Chargement des matrices de projection après rectification fournies pour
% les deux caméras
calibration = loadCalibrationCamToCam('calib_cam_to_cam.txt');
P1 = calibration.P_rect{3};
P2 = calibration.P_rect{4};

% Chargement des images
im1 = imread('image_02\0000000040.png');
im2 = imread('image_03\0000000040.png');
...
% [ r1, c1] = detectionPointsInteret(im1,10000,5,3,'gradient');
% [ r2, c2] = detectionPointsInteret(im2,10000,5,3,'gradient');
...
% Suprression des points en z négatifs. Cette fois-ci les tests ont
% montré que les z positifs doivent être gardés
mat = z<0;
idx = find(mat);
x(idx) = [];
y(idx) = [];
...
pcwrite(ptCloud,'points_stereo.ply');
